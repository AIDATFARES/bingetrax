...
export default function LatestArticlesSection() {
  ...
  return (
    <section className="py-20 bg-white text-black relative z-10 border-t border-black/10 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-primary-600/5 rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <Link href={`/blog/${post.slug}`} key={post.id}>
              <article className="bg-white rounded-2xl overflow-hidden flex flex-col group cursor-pointer hover:-translate-y-2 transition-all duration-300 h-full border border-black/10 hover:border-[#7000FF]/50">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <h3 className="text-lg font-bold leading-snug text-black group-hover:text-[#7000FF] transition-colors mb-3 line-clamp-2">
                    {post.title}
                  </h3>
                  ...
                  <div className="mt-auto text-slate-500 text-xs font-medium flex items-center justify-between border-t border-black/10 pt-4">
                    <span>{post.date}</span>
                    <span className="flex items-center text-[#7000FF] group-hover:translate-x-1 transition-transform font-bold text-sm">
                      Read More
        ...
        <div className="text-center">
          <Link href="/blog">
            <button className="bg-black/5 hover:bg-[#7000FF] text-black border border-black/15 hover:border-[#7000FF] px-8 py-3.5 rounded-full font-bold text-sm transition-all duration-300 flex items-center justify-center mx-auto gap-2">
              View All Articles
